...
function Divider() {
	return <div className='w-[1px] bg-gray-400 mx-1' />;
}
...
function FloatingLinkEditor({ editor }: { editor: LexicalEditor }) {
	...
	const updateLinkEditor = useCallback(() => {
		...
		if (
			...
			rootElement !== null &&
			rootElement.contains(nativeSelection?.anchorNode ? nativeSelection?.anchorNode : null)
		) {
			...
			!activeElement ||
			activeElement.className !== 'editor-input'
			// 'block w-[calc(100%_-_24px)] box-border bg-[#eee] text-[15px] text-[rgb(5,5,5)] relative mx-3 my-2 px-3 py-2 rounded-[15px] border-0'
		) {
			// editor-input a?
